...
| Chart bar primary | `color.chart.bar.primary` | `#f4f4f4` | Pulse Recharts marketplace volume and listing-depth bars | `OM-PROT-004-stats`, prototype only |
| Chart bar secondary | `color.chart.bar.secondary` | `#858585` | Reserved for inactive or comparison chart bars, not the default Stats graph series | `OM-PROT-004-stats`, prototype only |
| Chart cursor | `color.chart.cursor` | `rgba(255, 255, 255, 0.06)` | Neutral Recharts hover/focus bucket cursor | `OM-PROT-004-stats`, prototype only |
...
- Stats panel distribution: use the Paper network panels as the sizing reference. Chart rows should be continuous two-column bands with 298 px panel height, sparse body space, large headline metrics only in the header, and bottom metadata anchored to the panel edge. The local marketplace graphs use a tokenized Recharts bar adapter for 12 2-hour volume buckets and floor-band listing depth, with neutral hover/cursor treatment and no animated/pulsing series bars. Three-up summary rows should be one bordered band with equal columns and vertical dividers, not separate cards with gaps.
...
- Chart surface: a reserved black plot area with subtle grid lines, right-side y-axis text, bottom bucket labels, active timeframe border when controls are present, and red corner brackets where production confirms the token-detail chart frame. Recharts surfaces must inherit Orb chart tokens instead of generic `--chart-*` defaults or shadcn-style `hsl(var(--chart-1))` fills.
