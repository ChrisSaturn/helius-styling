...
10. [Network Stats Capture](./network-stats.md) - production Stats/Network screen capture for recent blocks, validators, and epoch state.
11. [Pulse Prototype](./pulse.md) - intentional NFT monitor prototype guidance plus local Stats implementation alignment.
12. [Private Payments API Prototype](./private-payments-api.md) - Paper-created landing page concept based on solana.new and user-supplied payments API copy.
...
- Network stats production evidence is captured in [network-stats.md](./network-stats.md), covering recent blocks, TPS/SOL chart panels, validator summary panels, active validators, and epoch state from Paper node `1ZQ-0`.
- AI token analysis popup production evidence is captured in [ai-analysis-popup.md](./ai-analysis-popup.md).
- Private Payments API prototype guidance is captured in [private-payments-api.md](./private-payments-api.md), with Paper artboard `3JV-0` using the current styleguide for a solana.new-like payments API landing page and a local Vite implementation in `../eclipse (private-payments)`.
- Pulse NFT monitor, `Me` portfolio, NFT-only local `Stats` marketplace implementation, and placeholder Settings popup prototype guidance is captured in [pulse.md](./pulse.md). Pulse now includes capped latest sales/listings placeholders, derived monitor metrics and incoming-data animations that refresh from local mock data every 4.5 seconds, NFT marketplace stats across listing platforms, Paper-aligned continuous marketplace volume/depth graph and summary bands, and browser metadata wired to the `winniepoo-mert/favicon_io` favicon bundle through the Vite public asset path.
- Desktop top navigation from Paper node `2-0` is promoted as a reusable component in [Components](./03-components.md).
- Shared visual, component, page pattern, interaction, and implementation docs now include styling guidance from `OM-001-home`, `OM-005-token-detail`, `OM-008-network-stats`, `OM-009-ai-analysis-popup`, prototype-only monitor guidance from `OM-PROT-001-pulse`, prototype-only portfolio guidance from `OM-PROT-002-portfolio`, prototype-only settings guidance from `OM-PROT-003-settings`, local NFT Stats alignment guidance from `OM-PROT-004-stats`, and landing-page guidance from `OM-PROT-005-private-payments-api`, including the Pulse mock-feed plus marketplace mock-data boundaries for future stream, marketplace, DAS, and aggregator replacement.
