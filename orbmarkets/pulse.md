...
- `pulse/src/StatsRechartsBarChart.tsx` owns the Recharts-specific renderer for Pulse Stats, including `ResponsiveContainer`, `BarChart`, right-side `YAxis`, bottom `XAxis`, horizontal grid, square neutral bars, explicit variant bar sizes, custom tooltip, `accessibilityLayer`, chart-domain ticks, disabled series entry animation, and non-visual summaries. It is lazy-loaded from the Stats chart band so Recharts does not enter the initial Pulse monitor bundle.
...
- `pulse/src/styles.css` should align to the updated Orb rules with a shared 1480 px site shell, 18/14/12 px horizontal gutters, 69 px desktop navigation bar, 61 px search rail, square 0 px visible UI corners, Pulse logo sizing in the brand rail, anchored settings popup, profile summary band, one bordered KPI band, Paper-aligned continuous Stats chart and summary bands, neutral tokenized Recharts chart surfaces/tooltips, 64 px table rows, fixed 40 px neutral NFT thumbnails, restrained hover states, selected text fill for secondary states, Lucide icon sizing rules, red-orange reserved for brand/main active/focus treatment, table-panel `content-visibility`, native table-cell preservation, incoming row and KPI refresh animations outside chart bars, and reduced-motion handling.
...
- `03-components.md` records the monitor KPI strip, NFT event table, NFT identity cell, portfolio components, Recharts chart adapter, NFT marketplace stats components, settings popup implementation constraints, selected text-fill rules, White/primary text treatment for collection names and main clickables, 12-bucket marketplace volume chart constraints, mock-feed metric derivation, and shallow surface composition rules.
